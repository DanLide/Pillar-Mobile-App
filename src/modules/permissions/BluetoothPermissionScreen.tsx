...
export const BluetoothPermissionScreen = observer(({ navigation, route }: Props) => {
  const [bluetoothPermissionChecked, setBluetoothPermissionChecked] = useState<boolean>(false);
  console.warn(permissionStore.bluetoothPermission)
  const onButtonPress = async () => {
    if (permissionStore.bluetoothPermission === RESULTS.DENIED) {
      const result = await permissionStore.requestPermission(PERMISSIONS.IOS.BLUETOOTH_PERIPHERAL);
      if (result === RESULTS.GRANTED) {
        navigation.replace(route.params.nextRoute, { succeedBluetooth: true });
      }
    ...
    navigation.navigate(AppNavigator.SelectStockScreen, { succeedBluetooth: false });
  };
    ...
    <View style={styles.container}>
      <View style={styles.contentContainer}>
        ...
        <Text style={styles.subtitle}>
          RepairStack would like to connect to your bluetooth, so that you can easily unlock cabinets at your shop.
        </Text>
