...
class MasterLockStore {
  @observable masterLock: MasterLockInterface;
  ...
  constructor() {
    ...
    this.masterLock = masterLockModuleStore;
    this.stocksState = {};
    this.isUnlocking = false;
    this.masterLock.addListener('visibilityStatus', (data: string) => {
    ...
    this.masterLock.addListener('lockStatus', (data: string) => {
    ...
    this.masterLock.configure(LICENSE_ID);
  }
  ...
  @action unlock(deviceID: string) {
    ...
    this.masterLock.unlock(deviceID).then(() => {
      runInAction(() => {
        this.isUnlocking = false;
      });
    });
